(function () {
  ...
  function scheduleEnhance() {
    ...
    enhanceQueued = true;
    requestAnimationFrame(() => { enhanceQueued = false; enhanceDevicePage(); });
  }
  ...
  function openWizard() { injectStyles(); createDialog(); showStart(); }
  document.addEventListener('click', (event) => {
    const button = event.target.closest?.('button[data-mom-provisioning="true"]'); if (!button) return;
    event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation(); openWizard();
  }, true);
  injectStyles(); scheduleEnhance(); new MutationObserver(scheduleEnhance).observe(document.body, { childList:true, subtree:true }); window.addEventListener('popstate', scheduleEnhance); window.MOMDeviceProvisioning = { open:openWizard };
})();
